...
from electrumx.lib.util import OldTaskGroup, class_logger, json_deserialize
...
class PeerManager:
    ...
    async def _detect_proxy(self):
        ...
        while True:
            self.logger.info(f'trying to detect proxy on "{host}" ' f"ports {ports}")
    ...
    async def _should_drop_peer(self, peer):
        ...
        for kind, port, family in peer.connection_tuples():
            ...
            try:
                ...
                self.logger.error(f"{peer_text} RPC error: {e.message} " f"({e.code})")
    ...
    async def _verify_peer(self, session, peer):
        ...
        async with OldTaskGroup() as g:
            ...
            peers_task = await g.spawn(self._send_peers_subscribe(session, peer))
    ...
    async def _send_headers_subscribe(self, session):
        ...
        if ours != theirs:
            raise BadPeerError(f"our header {ours} and " f"theirs {theirs} differ")
    ...
    def on_peers_subscribe(self, is_tor):
        ...
        cutoff = time.time() - STALE_SECS
        peers = {myself for myself in self.myselves if myself.last_good > cutoff}
