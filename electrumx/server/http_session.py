...
class HttpHandler(object):
    ...
    def atomicals_search_name_template(self, db_prefix, name_type_str, parent_prefix=None, prefix=None, Reverse=False, Limit=100, Offset=0, is_verified_only=False):
        ...
        for item in db_entries:
            ...
            elif name_type_str == "protocol":
                status, _, _ = self.session_mgr.bp.get_effective_name_template(b'pr', item['name'], self.session_mgr.bp.height, self.session_mgr.bp.protocol_data_cache)
        ...
        for returned_utxo in returned_utxos:
            for atomical_id_entry_compact in returned_utxo['atomicals']:
                ...
                if return_struct['atomicals'].get(atomical_id_ref) is None:
                    ...
                    elif atomical_id_basic_info.get('$protocol'):
                        return_struct['atomicals'][atomical_id_ref]['subtype'] = atomical_id_basic_info.get('subtype')
                        return_struct['atomicals'][atomical_id_ref]['request_protocol_status'] = atomical_id_basic_info.get('$request_protocol_status')
                        return_struct['atomicals'][atomical_id_ref]['protocol'] = atomical_id_basic_info.get('$protocol')
                        return_struct['atomicals'][atomical_id_ref]['request_protocol'] = atomical_id_basic_info.get('$request_protocol')
                    elif atomical_id_basic_info.get('$contract'):
                        return_struct['atomicals'][atomical_id_ref]['subtype'] = atomical_id_basic_info.get('subtype')
                        return_struct['atomicals'][atomical_id_ref]['request_contract_status'] = atomical_id_basic_info.get('$request_contract_status')
                        return_struct['atomicals'][atomical_id_ref]['contract'] = atomical_id_basic_info.get('$contract')
                        return_struct['atomicals'][atomical_id_ref]['request_contract'] = atomical_id_basic_info.get('$request_contract')
                        return_struct['atomicals'][atomical_id_ref]['instance_of_protocol'] = atomical_id_basic_info.get('$instance_of_protocol')
                        return_struct['atomicals'][atomical_id_ref]['instance_of_protocol_id'] = atomical_id_basic_info.get('$instance_of_protocol_id')
    ...
    async def atomicals_search_protocols(self, request):
        params = await self.format_params(request)
        prefix = params.get(0, None)
        Reverse = params.get(1, False)
        Limit = params.get(2, 100)
        Offset = params.get(3, 0)
        is_verified_only = params.get(4, True)
        return self.atomicals_search_name_template(b'pr', 'protocol', None, prefix, Reverse, Limit, Offset, is_verified_only)
