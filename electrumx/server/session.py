...
import asyncio
from cbor2 import loads, CBORDecodeError
import attr
import pylru
        ...
        for service in services:
            ...
            if service.protocol == 'http':
                ...
                try:
                    ...
                    app.router.add_get('/proxy/blockchain.atomicals.find_protocols', handler.atomicals_search_protocols)
                    ...
                    app.router.add_post('/proxy/blockchain.atomicals.find_protocols', handler.atomicals_search_protocols)
...
class ElectrumX(SessionBase):
    ...
    async def atomicals_get_by_protocol(self, name):
        ...
        height = self.session_mgr.bp.height
        status, candidate_atomical_id, all_entries = self.session_mgr.bp.get_effective_protocol(name, height)
        formatted_entries = format_name_type_candidates_to_rpc(all_entries, self.session_mgr.bp.build_atomical_id_to_candidate_map(all_entries))
    ...
    async def atomicals_eval_contract(self, contract_cbor):
        # contract_data is the cbor data
        contract_data = loads(contract_cbor)
        name = contract_data.get('n')
        method = contract_data.get('m')
        args = contract_data.get('args')
    ...
    async def atomicals_search_protocols(self, prefix=None, Reverse=False, Limit=100, Offset=0, is_verified_only=True):
        if isinstance(prefix, str):
            prefix = prefix.encode()
    ...
    def set_request_handlers(self, ptuple):
        ...
        handlers = {
            ...
            'blockchain.atomicals.get_by_protocol': self.atomicals_get_by_protocol,
            'blockchain.atomicals.get_by_contract': self.atomicals_get_by_contract,
            'blockchain.atomicals.eval_contract': self.atomicals_eval_contract,
