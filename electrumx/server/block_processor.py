...
from electrumx.lib.util import (
    chunks, class_logger, pack_le_uint32, pack_be_uint32, unpack_le_uint32, pack_le_uint64, unpack_le_uint64, pack_be_uint64, unpack_be_uint64, OldTaskGroup, pack_byte, pack_le_uint16, unpack_le_uint16_from
)
import math
...
class BlockProcessor:
    ...
    def flush_data(self):
        ...
        return FlushData(self.height, self.tx_count, self.headers,
                         ...
                         self.op_data_cache, self.protocol_data_cache, self.reactor_data_cache, self.reactor_states_cache)
    ...
    def get_atomicals_id_mint_info(self, atomical_id, with_cache):
        ...
        if result:
            ...
            self.logger.info(f'result={result} for atomical_id={location_id_bytes_to_compact(atomical_id)}')
    ...
    def validate_and_create_nft_mint_utxo(self, mint_info, tx_hash):
    ...
    def create_or_delete_atomical(self, operations_found_at_inputs, atomicals_spent_at_inputs, header, height, tx_num, atomical_num, tx, tx_hash, Delete):
        ...
        if not valid_create_op_type or (valid_create_op_type != 'NFT' and valid_create_op_type != 'FT' and valid_create_op_type != 'PROTOCOL' and valid_create_op_type != 'CONTRACT'):
            return None
        ...
        self.logger.info(f'saving atomical...{mint_info}')
    ...
    def put_or_delete_init_state_updates(self, mint_info, data_payload, Delete):
        ...
        # The following deep copy was removed due to an error "TypeError: cannot pickle '_cbor2.CBORTag' object" payload error found in testnet
        ...
        # Save the init state only if there is anything to save
        if len(data_payload.keys()) > 0:
    ...
    # Get the effective name for realms, containers, tickers, protocols and contracts. Does NOT work for subrealms, use the get_effective_subrealm method directly
    def get_effective_name_template(self, db_prefix, subject, height, name_data_cache):
        ...
        if len(all_entries) > 0:
            ...
            self.logger.info(f'atomical_id={location_id_bytes_to_compact(atomical_id)}')
