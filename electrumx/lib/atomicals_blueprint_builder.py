...
class OutputItem:
   '''An output item'''
   def __init__(self, atomical_id, satvalue, tokenvalue, exponent):
...
def build_reverse_output_to_atomical_id_map(atomical_id_to_output_index_map):
    ...
    for atomical_id, output_info in atomical_id_to_output_index_map.items():
        for out_idx in output_info.expected_outputs:
            ...
            reverse_mapped[out_idx][atomical_id] = OutputItem(atomical_id, output_info.satvalue, output_info.tokenvalue, output_info.exponent)
...
def calculate_outputs_to_color_for_ft_atomical_ids(tx, ft_atomicals, target_exponent, sort_by_fifo):
    ...
    for item in atomical_list:
      ...
      if target_exponent != -1: 
        use_exponent = target_exponent
      sats_needed = get_adjusted_sats_needed_by_exponent(item.total_tokenvalue, use_exponent)
      cleanly_assigned, expected_outputs = AtomicalsTransferBlueprintBuilder.assign_expected_outputs_basic(sats_needed, tx, next_start_out_idx)
      if cleanly_assigned and len(expected_outputs) > 0:
        next_start_out_idx = expected_outputs[-1] + 1
        potential_atomical_ids_to_output_idxs_map[atomical_id] = ExpectedOutputSet(expected_outputs, item.max_exponent)
      else:
    ...
    if non_clean_output_slots:
      ...
      for item in atomical_list:
        atomical_id = item.atomical_id
        sats_needed = get_adjusted_sats_needed_by_exponent(item.total_tokenvalue, item.max_exponent)
        cleanly_assigned, expected_outputs = AtomicalsTransferBlueprintBuilder.assign_expected_outputs_basic(sats_needed, tx, 0)
        potential_atomical_ids_to_output_idxs_map[atomical_id] = ExpectedOutputSet(expected_outputs, item.max_exponent)
...
class AtomicalFtOutputBlueprintAssignmentSummary:
  def __init__(self, outputs, burned, first_atomical_id):
...
class AtomicalsTransferBlueprintBuilder:
  ...
  def calculate_output_blueprint_fts(cls, tx, ft_atomicals, operations_found_at_inputs, sort_fifo):
      if not ft_atomicals or len(ft_atomicals) == 0:
          return AtomicalFtOutputBlueprintAssignmentSummary({}, False, None) 
      ...
      if should_split_ft_atomicals:
          print(f'split_found ft_atomicals={ft_atomicals}')
          return AtomicalsTransferBlueprintBuilder.color_ft_atomicals_split(ft_atomicals, operations_found_at_inputs, tx)
     
      # A (re)substantiation operation was requested
      should_substantiate_ft_atomicals = is_substantiate_operation(operations_found_at_inputs)
      if should_substantiate_ft_atomicals:
        resubstantiated_result = AtomicalsTransferBlueprintBuilder.color_ft_atomicals_substantiate(ft_atomicals, operations_found_at_inputs, tx)
        # If the operation was successful return it, otherwise let it fall through to the regular assignment below
        if resubstantiated_result:
          return resubstantiated_result
  ...
  def color_ft_atomicals_split(cls, ft_atomicals, operations_found_at_inputs, tx):
    ...
    return AtomicalFtOutputBlueprintAssignmentSummary(output_colored_map, not cleanly_assigned, None) 

  @classmethod
  def color_ft_atomicals_regular(cls, ft_atomicals, tx, sort_fifo):
      output_colored_map = {}
      atomical_id_to_expected_outs_map, cleanly_assigned, atomicals_list_result = calculate_outputs_to_color_for_ft_atomical_ids(tx, ft_atomicals, -1, sort_fifo)
      ...
      for atomical_id, atomical_info in atomical_id_to_expected_outs_map.items():
      ...
      return AtomicalFtOutputBlueprintAssignmentSummary(output_colored_map, not cleanly_assigned, first_atomical_id) 
  
  @classmethod
  def color_ft_atomicals_substantiate(cls, ft_atomicals, operations_found_at_inputs, tx):
      if not is_substantiate_operation(operations_found_at_inputs):
        raise IndexError('Developer Error. Should be checked before')
      
      if not operations_found_at_inputs.get('payload') or not isinstance(operations_found_at_inputs.get('payload'), dict):
        return None 
      
      target_exponent = operations_found_at_inputs['payload'].get('e')
      if not isinstance(target_exponent, int) or target_exponent < 0 or target_exponent > 8:
        return None 
       
      output_colored_map = {}
      atomical_id_to_expected_outs_map, cleanly_assigned, atomicals_list_result = calculate_outputs_to_color_for_ft_atomical_ids(tx, ft_atomicals, target_exponent, True)
      first_atomical_id = None 
  ...
  def are_payments_satisfied(self, expected_payment_outputs):
    ...
    # Calculate an alternative target exponent if it was requested to substantiage in the same as the payment tx
    target_exponent = -1
    if is_substantiate_operation(self.operations_found_at_inputs):
      e = self.operations_found_at_inputs.get('payload').get('e')
      if isinstance(e, int) and e > 0 and e <= 8:
        target_exponent = e

    # Prepare the mapping of which ARC20 is paid at which output
    atomical_id_to_output_index_map, ignore, atomicals_list_result_ignore = calculate_outputs_to_color_for_ft_atomical_ids(self.tx, self.ft_atomicals, target_exponent, self.sort_fifo)
    ...
    for idx, txout in enumerate(self.tx.outputs):
      ...
      if not expected_output_payment_id_type:
        ...
        if output_summary and output_summary.get(expected_output_payment_id_type_long_form):
          # Ensure the normalized tokenvalue is greater than or equal to the expected payment amount in that token type
          output_summary_for_atomical_id = output_summary.get(expected_output_payment_id_type_long_form)
          tokenvalue = output_summary_for_atomical_id.tokenvalue
