...
class AtomicalsTransferBlueprintBuilder:
  ...
  def __init__(self, atomicals_spent_at_inputs, operations_found_at_inputs, tx, get_atomicals_id_mint_info, sort_fifo):
    ...
    nft_output_blueprint, ft_output_blueprint = self.__calculate_output_blueprint(self.tx, self.nft_atomicals, self.ft_atomicals, self.atomicals_spent_at_inputs, self.operations_found_at_inputs, self.sort_fifo)
  ...
  def __calculate_output_blueprint_nfts(self, tx, nft_atomicals, atomicals_spent_at_inputs, operations_found_at_inputs, sort_fifo):
  ...
  def __calculate_output_blueprint_fts(self, tx, ft_atomicals, atomicals_spent_at_inputs, operations_found_at_inputs, sort_fifo):
      ...
      if should_split_ft_atomicals:
          return self.__color_ft_atomicals_split(ft_atomicals, operations_found_at_inputs, tx)
      else:
          return self.__color_ft_atomicals_regular(ft_atomicals, operations_found_at_inputs, tx, sort_fifo)

  def __color_ft_atomicals_split(self, ft_atomicals, operations_found_at_inputs, tx):
      output_colored_map = {
        'outputs': {
        },
        'burned': False,
  ...
  def __color_ft_atomicals_regular(self, ft_atomicals, operations_found_at_inputs, tx, sort_fifo):
      output_colored_map = {}
      ...
      for atomical_id, outputs_to_color_info in atomical_id_to_expected_outs_map.items():
        for expected_output_index in outputs_to_color_info['expected_outputs']
            output_colored_map[out_idx]['atomicals'][atomical_id] = {
                'exponent': outputs_to_color_info['exponent'],  
                'satvalue': txout.value,                      # Adds expected satvalue for sanity check
                'info': atomical_info
              }
      return {
        'outputs': output_colored_map,
        'burned': not cleanly_assigned,
        'first_atomical_id': first_atomical_id
      }

  def __calculate_output_blueprint(self, tx, nft_atomicals, ft_atomicals, atomicals_spent_at_inputs, operations_found_at_inputs, sort_fifo):
      nft_blueprint = self.__calculate_output_blueprint_nfts(tx, nft_atomicals, atomicals_spent_at_inputs, operations_found_at_inputs, sort_fifo)
      ft_blueprint = self.__calculate_output_blueprint_fts(tx, ft_atomicals, atomicals_spent_at_inputs, operations_found_at_inputs, sort_fifo)
