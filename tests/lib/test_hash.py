...
def test_double_sha256():
    assert (
        lib_hash.double_sha256(b"double_sha256")
...
def test_Base58_decode_check_custom():
    decode_check_sha256 = partial(lib_hash.Base58.decode_check, hash_fn=lib_hash.sha256)
    with pytest.raises(TypeError):
...
def test_Base58_encode_check_custom():
    encode_check_sha256 = partial(lib_hash.Base58.encode_check, hash_fn=lib_hash.sha256)
    with pytest.raises(TypeError):
